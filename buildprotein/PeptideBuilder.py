...
def get_np_distance(p1, p2):
    return np.linalg.norm(p1 - p2)

def get_np_angle(p1, p2, p3):

    v1 = p1 - p2
    v2 = p3 - p2

    v1_norm = get_np_norm(v1)
    v2_norm = get_np_norm(v2)
    
    c = np.sum(v1*v2, -1)/(v1_norm * v2_norm)

    c = np.clip(c, -0.999999, 0.999999)
    
    return np.arccos(c)/np.pi*180

def get_np_angle2(v1, v2):
...
def get_np_dihedral(p1, p2, p3, p4):
    ...
    return np.sign(np.sum(v3*c2, -1))*get_np_angle2(v1,v2)
...
def calculateCoordinates(c1, c2, c3, L, ang, di):
    ...
    ang = tf.cast(ang, tf.float32)
    di = tf.cast(di, tf.float32)
    ...
    bc = tf.linalg.normalize(bc)[0]
    n = tf.linalg.cross(ab, bc)
    
    n = tf.linalg.normalize(n)[0]
    ab = tf.linalg.cross(n, bc)
    
    mtr = tf.stack([-bc, ab, n])
    mtr = tf.transpose(mtr)

    bc = tf.reduce_sum(tf.multiply(mtr, d2), -1)
    cc = c3 + bc
    
    return tf.cast(cc, tf.float32)

def get_norm_batch(v):
    return tf.norm(v, axis=-1, keepdims=True)

def calculateCoordinates_batch(c1, c2, c3, L, ang, di):
    ...
    d2 = tf.stack([L*tf.math.cos(ang/180*np.pi),
                   L*tf.math.cos(di/180*np.pi)*tf.math.sin(ang/180*np.pi),
                   L*tf.math.sin(di/180*np.pi)*tf.math.sin(ang/180*np.pi)], -1)
    d2 = tf.expand_dims(d2, axis=-1)
    
    ab = c2 - c1
    bc = c3 - c2
    bc = bc/get_norm_batch(bc)
    n = tf.linalg.cross(ab, bc)
    n = n/get_norm_batch(n)
    ab = tf.linalg.cross(n, bc)
    
    mtr = tf.stack([-bc, ab, n])
    mtr = tf.transpose(mtr, [1,2,0])

    bc = tf.squeeze(tf.matmul(mtr, d2), -1)
    cc = c3 + bc
...
def get_sidechain(rotamer, residue, geo, atoms_matrix):
    ...
    side_atoms.extend(atoms)
    side_atoms_names.extend(all_atoms_name)
    
    return side_atoms, side_atoms_names