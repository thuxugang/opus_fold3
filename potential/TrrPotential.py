...
def cal_TrrPotential(Trr_matrix, atoms_matrix):
    ...
    dist_w = tf.gather(dist_w, case_dist)
    ...
    dist_potential = tf.reduce_mean(
        dist_w*(c_params[:,0]*tf.math.pow(dij-c_x_base,3) + c_params[:,1]*tf.math.pow(dij-c_x_base,2) + \
    ...
    omega_w = tf.gather(omega_w, case_omega)
    ...
    c_index = np.clip(c_index, 0, 26)
    c_x_base = bins_omega[c_index]
    num = c_index.shape[0]
    c_params = omega_c.numpy()[range(num), c_index, :]
    
    omega_potential = tf.reduce_mean(
        omega_w*(c_params[:,0]*tf.math.pow(omegaij-c_x_base,3) + c_params[:,1]*tf.math.pow(omegaij-c_x_base,2) + \
    ...
    theta_w = tf.gather(theta_w, case_theta)
    ...
    theta_potential = tf.reduce_mean(
        theta_w*(c_params[:,0]*tf.math.pow(thetaij-c_x_base,3) + c_params[:,1]*tf.math.pow(thetaij-c_x_base,2) + \
    ...
    phi_w = tf.gather(phi_w, case_phi)
    ...
    c_index = np.clip(c_index, 0, 14)
    c_x_base = bins_phi[c_index]
    num = c_index.shape[0]
    c_params = phi_c.numpy()[range(num), c_index, :]
    
    phi_potential = tf.reduce_mean(
        phi_w*(c_params[:,0]*tf.math.pow(phiij-c_x_base,3) + c_params[:,1]*tf.math.pow(phiij-c_x_base,2) + \
...
def init_MCTrr_matrix(residuesData, trr_cons, len_a=0):
    ...
    for i, j, p, c in dist_cons:
        res_i_cb = residuesData[i].main_chain_atoms_matrixid["CB"]
        res_j_cb = residuesData[j].main_chain_atoms_matrixid["CB"]
        dist_i_cb.append(res_i_cb)
        dist_j_cb.append(res_j_cb)
        dist_p.append(p)
        dist_c.append(c)
        if (i < len_a and j >= len_a) or (i >= len_a and j < len_a):
            w = weight
        else:
            w = 1
        dist_w.append(w)
    ...
    for i, j, p, c in omega_cons:
        res_i_ca = residuesData[i].main_chain_atoms_matrixid["CA"]
        res_i_cb = residuesData[i].main_chain_atoms_matrixid["CB"]
        res_j_ca = residuesData[j].main_chain_atoms_matrixid["CA"]
        res_j_cb = residuesData[j].main_chain_atoms_matrixid["CB"]
    ...
    Trr_matrix["dist_w"] = np.array(dist_w, dtype=np.float32)
    ...
    Trr_matrix["omega_w"] = np.array(omega_w, dtype=np.float32)
    ...
    Trr_matrix["theta_w"] = np.array(theta_w, dtype=np.float32)
    ...
    Trr_matrix["phi_w"] = np.array(phi_w, dtype=np.float32)
   
    return Trr_matrix

def init_SCTrr_matrix(residuesData, trr_cons, len_a=0, dihedral_index=0):
    ...
    dist_w = []
    for i, j, p, c in dist_cons:
        res_i_cb = get_pcb_counter(residuesData[i], dihedral_index)
        res_j_cb = get_pcb_counter(residuesData[j], dihedral_index)
        dist_i_cb.append(res_i_cb)
        dist_j_cb.append(res_j_cb)
        dist_p.append(p)
        dist_c.append(c)
...
def readTrrCons(path, resgap=None):
    ...
    for a,b,p in zip(i,j,prob):
        if b>a:
            if resgap == None or abs(a-b) <= resgap:
